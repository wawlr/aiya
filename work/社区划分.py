...
def constructG(fileName):
    ...
    with open('F:/classify_data/newMaterials/title_fos/'+fileName, 'r') as f:
        for position, line in enumerate(f):
            t = line.strip().split(' ')
            G.add_edge(t[0], t[1], weight = int(t[2]))
    return G
...
def divideCommunity(G):
    ...
    while True:
        ...
        if isMove == 0:
            break
    allQ = 0
    for community in list(communityNodeDict.keys()):
...
file_year_list = ["06","07","08","09","10","11","12","13","14","15","16","17"]
for file in file_year_list:
    input_path = "title_fos"+file+"_both_words_result.txt"

    G = constructG(input_path)
    print('construct network finished')
    begin = time.clock()
    realcommunity = mainFunc(G)
    # print(len(max(realcommunity.items(), key=lambda x: len(x[1]))[1]))
    print(time.clock() - begin)

    output = open("F:/classify_data/newMaterials/title_fos/title_fos"+file+"_both_words_new_result.txt", 'w')
    n = 0
